...
ggplot(df, aes(x, probabilidad)) +
  ...
  geom_vline(aes(xintercept = 6), linetype = "dashed", color = "black", size = 1) +
  annotate("text", x = 6, y = max(df$probabilidad)/2, label = "P(X <= 6)", color = "black") +
...
ggplot(df, aes(x, probabilidad)) +
  ...
  geom_vline(aes(xintercept = 11), linetype = "dashed", color = "orange", size = 1) +
  annotate("text", x = 11, y = max(df$probabilidad)/2, label = "P(X >= 12)", color = "orange") +
  labs(x = "X", y = "P(X)", title = "Distribución Binomial")
```
